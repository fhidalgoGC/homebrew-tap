...
description: Agrega un ADR (Architecture Decision Record) al doc `decisions` de la feature — nivel FEATURE. Para ADRs locales a una feature (aplican SÓLO a esa feature). Numeración global. Doc living. Para ADRs transversales al producto usar `/fremi-product-adr`. Para ADRs locales a una story usar `/fremi-story-adr`.
---

> **Nota sobre identificadores:** los prefijos concretos (carpeta feature, formato de ADR, formato de task) salen de `~/.fremi/framework/settings/methodology.core.yaml`. Este archivo usa conceptos semánticos y step IDs. Ver `.claude/rules/no-hardcoded-identifiers.md`.

# /fremi-feature-adr — Agregar ADR local a una feature

Agrega un nuevo Architecture Decision Record al doc `decisions` de la carpeta feature (`docs/works/features/{feature_folder}/decisions.md`). Es para decisiones que aplican **SÓLO** a esa feature.
...
**Numeración global** al proyecto — se calcula tomando el máximo ID existente + 1 sobre TODOS los decisions.md del proyecto.
...
```
/fremi-feature-adr <FEATURE_ID> [título]
```

- `<FEATURE_ID>`: ID de la feature padre (obligatorio, ej. con defaults: `FT-01`). El formato sale de `identifiers.feature.id_format`.
...
4. La IA referencia el ID asignado desde el artefacto donde nació la decisión (`aplica <adr-id>` en una línea del step `sdd` o `design`).
...
**El ADR es global al proyecto** (`adr_cfg.scope === "global"`). Aún si el ADR vive en el doc `decisions` de una feature, su número es global.
...
2. Extraer todos los IDs que matcheen `adr_cfg.regex`.
3. Próximo número = max(existentes) + 1. Si no hay → 1.
4. Construir el `id` aplicando `adr_cfg.id_format`.
...
El doc `decisions` (a nivel producto o feature) es un doc **living**. Consultar `config.yaml → versioning.parent_bump_triggers.adr_accepted`:

- **ADR nuevo aceptado** → **MINOR** bump del `decisions.md` destino.
- **ADR reemplaza otro** (marcado como reemplazado) → **MAJOR** bump.
...
- Si requiere actualizar alguna story relacionada (mencionar en su `{workflow.definition}` o `{workflow.design}` que aplica el ADR).

## Reglas

- **Numeración global** del proyecto. Aún si el ADR vive en la carpeta feature, el número es global.
- **No reciclar** IDs. Un ADR reemplazado se marca como reemplazado y se crea uno nuevo.
