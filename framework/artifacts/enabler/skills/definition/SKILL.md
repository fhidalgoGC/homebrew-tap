...
> **Nota sobre identificadores:** los prefijos concretos (carpeta enabler, IDs de feature/story) salen de `~/.fremi/framework/settings/methodology.core.yaml`. Este archivo usa step IDs semánticos (`definition`, `design`, `plan`, `closure`) y conceptos. Ver `.claude/rules/no-hardcoded-identifiers.md`.

# /fremi-enabler-definition — Poblar el doc `definition` del enabler
...
```
/fremi-enabler-definition <ENABLER_ID>
```

- `<ENABLER_ID>`: ID del enabler (ej. con defaults: `EN-02`). Debe existir con scaffold creado por `/fremi-enabler`. El formato sale de `identifiers.enabler.id_format`.

## Cuándo invocarlo

- `/fremi-enabler <nombre>` acaba de crear el scaffold y hay que rellenar el step `definition`.
...
- Placement válido: `docs/works/enablers/{enabler_folder}/` (global) o dentro de feature/story según scope.
...
- **Vinculaciones**: listar features/stories que dependen (referencia por `<FEATURE_ID>` o `<FEATURE_ID>/<STORY_ID>`).
...
- No bumpear al escribir; se rellena `version_at_closure` al firmar el step `closure`.
...
- ❌ Enabler sin vinculaciones → no habilita nada, es tooling (extra).
