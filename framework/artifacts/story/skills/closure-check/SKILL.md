...
- Antes de firmar el `{workflow.closure}` de una story.
...
4. Extraer de `config.yaml`:
   - `config.story.yaml → conditional_rules` — criterios que definen cuándo `{workflow.explore}` y `{workflow.proposal}` deben existir.
...
| `design` | `{workflow.design}` | Siempre | Librerías elegidas, componentes/módulos, contratos internos, patrones |
| `tdd-plan` | `{workflow.tdd}` | Siempre | Items con estado de checkbox (IDs según `identifiers.test_case.regex`) |
| `plan` | `{workflow.plan}` | Siempre | Tareas con estado y criterios (IDs según `identifiers.task.regex`) |
| `checkwork` | `{workflow.checkwork}` | Siempre | % progreso, tasks cerradas, CAs cubiertos, archivos implementados |
| `closure` | `{workflow.closure}` | Siempre | Si existe, parsear matriz de trazabilidad |

**Sobre los condicionales**: ausencia de `{workflow.explore}` o `{workflow.proposal}` **NO es un gap** si el criterio de obligatoriedad de `config.yaml` no aplicaba a la story. El closure-check pregunta:
...
#### 3.3 Contratos SDD
Por cada contrato en `{workflow.sdd}` (endpoint, modelo, error code, etc.):
...
#### 3.4 Items de TDD
Por cada `- [ ]` (no marcado) en `{workflow.tdd}`:
...
#### 3.6 Scope respetado
Por cada item de **Out-of-scope** en `{workflow.scope}`:
...
Por cada componente/módulo/archivo declarado en `{workflow.design}`:
- Verificar que existe la pieza correspondiente en el código (archivo, clase, función, módulo).
- Verificar que los patrones declarados aparecen aplicados (ej: si `{workflow.design}` dice "Repository pattern para acceso a DB", existe una capa repository).
- **Gap si:** una pieza del diseño técnico no tiene contraparte en código.

#### 3.8 Tareas del plan completadas y verificables
Por cada tarea `T-XXX` en `{workflow.plan}`:
...
### Paso 4 — Verificar el `{workflow.closure}`

Si `{workflow.closure}` existe:
...
- Completar matriz en {workflow.closure}.
...
Para detectar items pendientes en `{workflow.tdd}`:
- Contar `- [ ]` vs `- [x]`.
