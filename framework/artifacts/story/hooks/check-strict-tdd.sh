#!/usr/bin/env bash
# ============================================================================
# Hook: check-strict-tdd — STORY-DOMAIN
# Tipo: PostToolUse
# Matcher (sugerido): { "tool_name": "Write",
#                       "file_path": "src/**/*.ts" (o el path de código del proyecto) }
#
# ────────────────────────────────────────────────────────────────────────────
# DOMAIN: story — este hook aplica durante la fase `apply` del workflow de
#                 story, donde se implementa código con precondición de los
#                 docs `tdd` + `sdd` completos.
#
# Reglas aplicadas:
#   Regla 2 (no código sin SDD+TDD) → ~/.fremi/framework/artifacts/story/rules/tdd.md
#   Regla 7 (test rojo primero)     → ~/.fremi/framework/artifacts/story/rules/tdd.md
# ────────────────────────────────────────────────────────────────────────────
#
# Propósito:
#   Si `config.core.yaml → testing.strict_tdd: true`, verificar que cuando se
#   escribe código de producción, existe (o coincide con) un archivo de test
#   asociado.
#
# Heurística simple:
