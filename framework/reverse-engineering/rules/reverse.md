...
> **Documento de referencia:** `~/.fremi/framework/reverse-engineering/flow.md`, `~/.fremi/framework/pipelines/reverse-*/PIPELINE.md`, `~/.fremi/framework/reverse-engineering/`.
>
...
- **1 flow**: `~/.fremi/framework/reverse-engineering/flow.md` con las 6 fases canónicas.
- **1 config**: `~/.fremi/framework/settings/config.reverse.core.yaml` con defaults, skills declarados y políticas transversales.
...
El proyecto puede setear en `~/.fremi/framework/settings/config.reverse.core.yaml`:
...
- `~/.fremi/framework/reverse-engineering/flow.md` — descripción narrativa del flujo canónico de reverse.
