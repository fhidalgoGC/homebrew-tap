...
1. Todo skill de `docs/frmwk/` se publique en `.claude/skills/` con nombre `fremi-<...>`, aunque el `name:` del `SKILL.md` tuviera drift.
...
- **Al clonar el proyecto** — sincronizar `.claude/skills/` con `docs/frmwk/skills/`.
- **Al importar el framework** a un proyecto nuevo (después de `/fremi-import-template`).
- **Cuando cambia la estructura** de `docs/frmwk/skills/` (nuevos sub-skills, refactor de carpetas).
...
1. Verificar que existe `docs/frmwk/skills/` — si no, abortar (no es un proyecto con framework instalado).
...
- `docs/frmwk/skills/` — orquestadores + sub-skills por capa + globales + tools.
- `docs/frmwk/reverse-engineering/` — reverse-*.
- `docs/frmwk/installs/` — este skill.
...
3. Determinar el path del target según la ubicación en el árbol:
   - Orquestador de capa: `docs/frmwk/skills/<capa>/`
   - Sub-skill: `docs/frmwk/skills/<capa>/skills/<sub>/`
   - Skill global: `docs/frmwk/skills/<name>/`
   - Reverse: `docs/frmwk/reverse-engineering/<name>/`
   - Install: `docs/frmwk/installs/<name>/`
...
1. Verificar que `CLAUDE.md` referencia `docs/frmwk/rules/workflow.md` (típicamente en la introducción).
...
1. Enumerar hooks de `docs/frmwk/hooks/*.sh`.
...
▶ Hooks:
  ...
  ✓ Hooks registrados: N / 10 (docs/frmwk/hooks/)
  ⚠ Sin registrar: <lista>
...
- Aborta si `docs/frmwk/skills/` no existe (no hay framework para instalar).
...
- ❌ Editar `.claude/skills/` a mano — este skill lo mantiene sincronizado con `docs/frmwk/skills/`.
...
- Skills del framework: `docs/frmwk/skills/` (jerárquico por capa — Regla 21).
- Hooks del framework: `docs/frmwk/hooks/` (Regla 23).
- Rules del framework: `docs/frmwk/rules/workflow.md` (cargado vía CLAUDE.md).
