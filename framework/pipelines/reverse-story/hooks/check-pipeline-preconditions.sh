#!/usr/bin/env bash
# ============================================================================
# Hook: check-pipeline-preconditions — PIPELINE-REVERSE-STORY DOMAIN
# Tipo: UserPromptSubmit
#
# Fires when the user submits a prompt containing "/fremi-pipeline-reverse-story".
...
#   y HU ID los pasa el usuario como argumentos al invocar el pipeline. El hook
...
# Detectar invocación del pipeline reverse-story
[[ "$PROMPT" != *"/fremi-pipeline-reverse-story"* ]] && exit 0
...
# --------------------------------------------------------------------------
# Upper-layer preconditions para pipeline-reverse-story:
...
#   exista; la validación del ID específico la hace el orquestador en fase 0.
# --------------------------------------------------------------------------
if [[ ! -d "docs/works/features" ]]; then
  problems+=("Carpeta docs/works/features/ no existe — reverse-story requiere que la feature padre exista. Corré /fremi-pipeline-reverse-feature primero para la feature correspondiente.")
fi

if [[ ${#problems[@]} -gt 0 ]]; then
  echo ""
  echo "⚠️  [pipeline-reverse-preconditions/story] Antes de invocar /fremi-pipeline-reverse-story:"
