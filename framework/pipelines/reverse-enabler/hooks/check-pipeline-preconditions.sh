#!/usr/bin/env bash
# ============================================================================
# Hook: check-pipeline-preconditions — PIPELINE-REVERSE-ENABLER DOMAIN
# Tipo: UserPromptSubmit
#
# Fires when the user submits a prompt containing "/fremi-pipeline-reverse-enabler".
...
# Detectar invocación del pipeline reverse-enabler
[[ "$PROMPT" != *"/fremi-pipeline-reverse-enabler"* ]] && exit 0
...
# --------------------------------------------------------------------------
# Upper-layer preconditions para pipeline-reverse-enabler (scope variable):
#   --scope global (default): ninguna upper-layer requerida.
#   --scope feature <feature-id>: la carpeta de la feature debe existir.
#   --scope story <feature-id>/<story-id>: la carpeta de la story debe existir.
#
# El hook extrae el scope del prompt para verificar la upper-layer correcta.
# Sólo emite warnings — la validación del ID específico la hace el orquestador.
# --------------------------------------------------------------------------

# Detectar si el prompt menciona --scope feature o --scope story
# Sólo verificamos la estructura general — el ID específico lo valida el pipeline.
if echo "$PROMPT" | grep -q -- '--scope story'; then
  if [[ ! -d "docs/works/features" ]]; then
    problems+=("Scope story declarado pero docs/works/features/ no existe — la story padre debe existir. Verificá el path o corré /fremi-pipeline-reverse-story primero.")
  fi
elif echo "$PROMPT" | grep -q -- '--scope feature'; then
  if [[ ! -d "docs/works/features" ]]; then
    problems+=("Scope feature declarado pero docs/works/features/ no existe — la feature padre debe existir. Verificá el path o corré /fremi-pipeline-reverse-feature primero.")
  fi
fi
...
if [[ ${#problems[@]} -gt 0 ]]; then
  echo ""
  echo "⚠️  [pipeline-reverse-preconditions/enabler] Antes de invocar /fremi-pipeline-reverse-enabler:"
