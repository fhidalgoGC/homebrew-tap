# `~/.fremi/framework/settings/` — Configuración runtime de la metodología
...
Los **hooks** (en `~/.fremi/framework/hooks/`) hoy no leen el JSON directamente — la convención está hardcoded en bash. Si querés que un hook respete cambios del JSON, hay que parsearlo con `jq` dentro del script.
...
- **No agregues acá reglas del flujo** (esas van a `~/.fremi/framework/rules/workflow.md`).
- **No agregues acá documentación general** (la metodología viva en `~/.fremi/framework/flows/workflow.md` y `CLAUDE.md`).
...
- **Comportamientos automatizados** (ej: validación al commit) → van a `~/.fremi/framework/hooks/` (no acá).
- **Skills nuevas** → van a `~/.fremi/framework/skills/<nombre>/SKILL.md` (no acá).
