...
description: Crea un skill ESPECÍFICO del proyecto bajo `docs/project/skills/<name>/` y lo expone al agente via symlink en `.claude/skills/<name>`. No toca `docs/frmwk/` (metodología). Reemplaza al viejo `/fremi-tools skill`.
---
...
**No invocarlo si**:
- El skill sería reusable en cualquier proyecto → editar `docs/frmwk/skills/` a mano.
...
3. Verificar que NO existen: `docs/project/skills/<name>/`, `.claude/skills/<name>`, ni `docs/frmwk/skills/<name>/` (colisión con framework).
...
- NO tocar `docs/frmwk/` ni `docs/works/`.
