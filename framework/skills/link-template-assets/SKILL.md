...
- `--remove` (opcional): **invierte la operación**. Desenlaza los assets que apuntan a `/template/` (no toca los de `docs/frmwk/` ni los creados con `/fremi-add-skill` / `/fremi-add-hook` / `/fremi-add-rule` / `/fremi-add-mcp`).
...
1. **Colisión con metodología (`docs/frmwk/<type>/<same-name>`):** si existe → **abortar todo el skill** (no procesa nada). Mostrar al usuario:
...
1. Verificar estado de `.claude/skills/`:
   - Si es symlink (estado heredado) → convertir a folder-of-symlinks (mismo procedimiento que `/fremi-add-skill` / `/fremi-add-hook` / `/fremi-add-rule` / `/fremi-add-mcp`):
     - `rm .claude/skills`
     - `mkdir .claude/skills`
     - Para cada subdir en `docs/frmwk/skills/`: `ln -s ../../docs/frmwk/skills/<frmwk-skill> .claude/skills/<frmwk-skill>`.
...
Cuando se invoca con `--remove`, el skill **invierte** el procedimiento de enlace. Es destructivo pero **acotado**: sólo toca symlinks que apunten al `/template/`. Nunca borra archivos reales, ni symlinks que apunten a otros destinos (`docs/frmwk/`, `docs/project/` con archivos reales creados por `/fremi-add-skill` / `/fremi-add-hook` / `/fremi-add-rule` / `/fremi-add-mcp`, etc.).
...
🔄 Estado de .claude/skills/: (si hubo conversión, mencionar)
   - Antes: symlink a docs/frmwk/skills.
...
- **Colisión con `docs/frmwk/<type>/<name>`** → abortar **todo el skill** (atómico); sugerir renombrar en el template.
...
- **No crear nada que toque `docs/frmwk/`, `docs/works/`, `/template/`** — esos quedan inmutables.
