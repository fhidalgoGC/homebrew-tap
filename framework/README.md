# `docs/frmwk/` — Configuración multi-agente del proyecto
...
```
.claude/rules    → docs/frmwk/rules         (symlink)
.claude/skills   → docs/frmwk/skills        (symlink)
.claude/hooks    → docs/frmwk/hooks         (symlink)
.claude/settings → docs/frmwk/settings      (symlink)
.cursor/...      → docs/frmwk/...           (symlink)
```
...
- ❌ Editar archivos a través de symlinks (`.claude/rules/...`) — siempre editar la fuente en `docs/frmwk/`.
- ❌ Duplicar configuración entre `.claude/` y `docs/frmwk/` (uno debe ser symlink del otro, nunca copia).
