# `docs/frmwk/plugins/` — Configuración de plugins por agente
...
1. Crear un archivo descriptivo: `docs/frmwk/plugins/<plugin-nombre>.json` (o el formato que pida el plugin).
2. **Documentarlo en este README** con:
   - Nombre y propósito.
   - Quién lo consume (qué agente lo lee).
   - Si necesita credenciales (en `.env` no versionado).
3. Wirearlo en el archivo de configuración del agente (`.claude/settings.json` o similar), apuntando a `docs/frmwk/plugins/<plugin-nombre>.json`.
